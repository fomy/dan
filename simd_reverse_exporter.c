...
/* Reverse a trace 
 * 	path: the input trace files
 * 	reverse_file: the output file
 * */
void reverse_trace(char **path, int n, char* reverse_file)
{
	char buf[4096];
	struct hashfile_handle *handle;
	const struct chunk_info *ci;
	/* keep track of all fingerprints */
	...
	for (; i < n; i++) {
		...
		while (1) {
			...
			while (1) {
				...
				/*if (hashsize <= 20 - sizeof(chunksize))*/
	...
	while (!g_sequence_iter_is_end(iter)) {
		write(fd, g_sequence_get(iter), 20);
 ...
 * 	path: path to the reverse trace
 */
void chunk_dedup_simd_trace(char *path, int weighted)
{
	...
	chunk.hashlen = 20;
	int byte = read(fd, chunk.hash, chunk.hashlen);
	while (byte > 0) {
...
void file_dedup_simd_trace(char* path, int weighted)
{
	...
	while (1) {
		int byte = read(fd, chunk.hash, 20);
		if(byte != 20)
			break;
...
int main(int argc, char *argv[])
{
	...
	if (reverse) {
		/* generate trace one by one */
		reverse_trace(&argv[optind], argc - optind,  reverse_file);
		return 0;
	}
